<?php
/* =============================================================
   楽園雅苑のサービス（page-service.php）

   スラッグが service の固定ページで、WordPress が自動的にこの
   テンプレートを使う。
   ...
   繰り返すので `.c-service-head` として共通化してある。

   本文（the_content()）は、温泉セクションの前に置くリード文として使う。
   管理画面の本文が空なら何も出ない。

   TODO: 掲載内容は Figma の実データをそのまま持っている。
         管理画面から編集したくなった場合は、カスタムフィールド等へ移すこと。
   ============================================================= */
            ...
            <div class="p-service__inner l-inner">
                ...
                <ul class="c-photo-grid c-photo-grid--2 p-service__photos--onsen">
                    <?php foreach ($onsen_photos as $p) : ?>
                        <li>
                            <img src="<?php echo esc_url($theme_uri . '/assets/images/service/' . $p['file'] . '.jpg'); ?>"
                                 alt="<?php echo esc_attr($p['alt']); ?>" width="580" height="370" loading="lazy">
                        </li>
                    <?php endforeach; ?>
                </ul>
                ...
                <dl class="c-table p-service__table">
                    <div class="c-table__row">
                        ...
                        <dd class="c-table__value c-table__value--long"><?php echo esc_html($onsen_quality); ?></dd>
                    </div>
                    <div class="c-table__row">
                        ...
                        <dd class="c-table__value c-table__value--long">
                            ...
                            <p class="c-table__note"><?php echo esc_html($onsen_effects_note); ?></p>
                        </dd>
                    </div>
                </dl>
            </div>
            ...
            <div class="p-service__inner l-inner">
                ...
                <ul class="c-photo-grid c-photo-grid--3">
                    <?php foreach ($room_photos as $p) : ?>
                        <li>
                            <img src="<?php echo esc_url($theme_uri . '/assets/images/service/' . $p['file'] . '.jpg'); ?>"
                                 alt="<?php echo esc_attr($p['alt']); ?>" width="380" height="280" loading="lazy">
                        </li>
                    <?php endforeach; ?>
                </ul>

                <div class="p-service__action">
                    <a class="c-btn-more" href="<?php echo esc_url(home_url('/#plan')); ?>">
                ...
                <div class="p-service__hours">
                    <?php foreach ($dining_hours as $line) : ?>
                        <p class="p-service__hours-line"><?php echo esc_html($line); ?></p>
                    <?php endforeach; ?>
